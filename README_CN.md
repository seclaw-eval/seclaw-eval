...
SPECSYNTH-CLAWBENCH 是 Benchmark for evaluating AI agent safety in synthetic openclaw tasks。它在独立 Docker 容器中运行 AI Agent 安全评测任务；每个任务包含 synthetic OpenClaw workspace、MCP 工具、mock service 和任务专属 grader。当前开源 release 包含 SeClaw-Bench 数据集，共 150 个 OpenClaw 任务，以及一个用于模型对比的 Docker 批量执行入口。
...
| [SeClaw-Bench Dataset Guide](docs/data.md) | SeClaw-Bench 任务清单、任务目录格式与数据集使用方式。 |
...
├── batch_inputs/                  # SeClaw-Bench 任务清单
...
# 4. 查看结果。
cat batch_logs/docker_eval/scores.json | python -m json.tool
open batch_logs/docker_eval/report.md
```
...
| 路径 | 说明 |
|------|------|
| `tasks/openclaw/` | SeClaw-Bench 的 150 个 OpenClaw 任务目录，每个任务可直接运行。 |
| `batch_inputs/task_list.jsonl` | SeClaw-Bench 公开任务清单，行内使用 `task_name`、`task_id` 和 `target`。 |
...
任务清单格式与目录约定见 [SeClaw-Bench Dataset Guide](docs/data.md)。Docker 环境配置与执行细节见 [Docker Evaluation Guide](docs/docker.md)。
